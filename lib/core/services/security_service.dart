...
class SecurityService {
  ...
  static validator.ValidationResult validateSecurityInput(
    String input,
    String type,
  ) {
  ...
  static Map<String, String> sanitizeUserData(Map<String, String> userData) {
    ...
    for (final entry in userData.entries) {
      sanitized[entry.key] = validator.InputValidator.sanitizeInput(
        entry.value,
      );
    }
  ...
  static void logSecurityEvent(String event, Map<String, dynamic> context) {
    if (kDebugMode) {
      ...
      context.forEach((key, value) {
        sanitizedContext[key] = NetworkSecurity.sanitizeErrorMessage(
          value.toString(),
        );
      });
