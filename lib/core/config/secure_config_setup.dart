...
class SecureConfigSetup {
  ...
  static Future<void> setupFromEnvironment() async {
    try {
      ...
      if (!isComplete) {
        if (kDebugMode) {
          debugPrint(
            'Environment configuration incomplete. Some features may not work.',
          );
          debugPrint(
            'Use SecureConfigSetup.setupManually() to configure manually.',
          );
        }
      } else {
        if (kDebugMode) {
  ...
  static Future<ConfigurationStatus> getConfigurationStatus() async {
    try {
      ...
      return ConfigurationStatus(
        ...
        isSupabaseKeyValid: EnvironmentConfig.isValidSupabaseAnonKey(
          supabaseKey,
        ),
      );
    } catch (e) {
    ...
    try {
      ...
      return {
        ...
        'nvidia_api_key': {
          ...
          'status': status.hasNvidiaKey && status.isNvidiaKeyValid
              ? 'OK'
              : 'MISSING/INVALID',
        },
        'supabase_url': {
          ...
          'status': status.hasSupabaseUrl && status.isSupabaseUrlValid
              ? 'OK'
              : 'MISSING/INVALID',
        },
        'supabase_anon_key': {
          ...
          'status': status.hasSupabaseKey && status.isSupabaseKeyValid
              ? 'OK'
              : 'MISSING/INVALID',
        },
      ...
      return {'error': 'Failed to generate checklist', 'details': e.toString()};
    }
  }
  ...
  static List<String> _generateRecommendations(ConfigurationStatus status) {
    ...
    if (!status.hasNvidiaKey || !status.isNvidiaKeyValid) {
      recommendations.add(
        'Set valid NVIDIA API key using EnvironmentConfig.setNvidiaApiKey()',
      );
    }

    if (!status.hasSupabaseUrl || !status.isSupabaseUrlValid) {
      recommendations.add(
        'Set valid Supabase URL using EnvironmentConfig.setSupabaseConfig()',
      );
    }

    if (!status.hasSupabaseKey || !status.isSupabaseKeyValid) {
      recommendations.add(
        'Set valid Supabase anon key using EnvironmentConfig.setSupabaseConfig()',
      );
    }
